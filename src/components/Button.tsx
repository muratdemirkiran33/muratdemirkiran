...
type Props = {
    ...
    className?: string;
    variant?: Variant;
    href?: string; // Add href manually since Link uses to
...
const Button = ({
    ...
    const buttonClasses = cn(
        `group h-12 px-8 inline-flex justify-center items-center gap-2 text-lg font-anton tracking-widest outline-none transition-colors relative overflow-hidden`,
        variantClasses,
        { [iconClasses]: icon },
        className,
    );
    ...
    if (as === 'link') {
        const props = rest as any; // Cast to any to handle href/to mismatch
        ...
        return (
            <Link className={buttonClasses} {...props} to={href}>
                {variant !== 'link' && (
                    <span className="absolute top-[200%] left-0 right-0 h-full bg-white rounded-[50%] group-hover:top-0 transition-all duration-500 scale-150"></span>
                )}
                <span className="z-[1]">
